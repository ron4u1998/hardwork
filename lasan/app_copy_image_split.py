...
def extract():
    ...
    if len(os.listdir(session['UPLOAD'])) == 0:
        ...
        for file in x:
            ...
            # im2 = im.crop((0, height / 2, width, height))
            # im1.save(os.path.join(session['UPLOAD'],'first_half.png'))
            # im2.save(os.path.join(session['UPLOAD'],'second_half.png'))
            # final_di = {
            #             "INVOICE_NO":       "None",
            #             "INVOICE_DATE":     "None", 
            #             "DELIVERY_NOTE":    "None", 
            ...
            # img_list = list()
            # di = dict()
            # for image in ['first_half.png','second_half.png']:
                # PredictFunc(image,session)
            model.predict(session['random_string']+'.png')
            model.decode(session['random_string']+'.png', file)
            os.remove(os.path.join(session['UPLOAD'],session['random_string']+'.png'))
            # img_list.append(img)
            # final_di.update(removeNone(di))
            # print(final_di)
            # print(os.path.join(session['OUTPUT'],file.split('.')[0]+'.json'))
            # with open(os.path.join(session['OUTPUT'],file.split('.')[0]+'.json'),'w') as json_file:
            #     json.dump(final_di, json_file)
            # img_list[0].save('final_1.png')
            # img_list[1].save('final_2.png')
            # get_concat_v(img_list[0], img_list[1]).save(os.path.join(session['OUTPUT'],file.split('.')[0]+'.png'))
            # PredictFunc(file,session)
            # os.remove(os.path.join(session['UPLOAD'],file))

            # inference_image.save(f"{session['OUTPUT']}/{image_path}")
            
        # for file in os.listdir(session['OUTPUT']):
        #     if file.endswith('.tsv'):
        #         os.remove(os.path.join(session['OUTPUT'], file))
        # shutil.rmtree(session['UPLOAD'])
