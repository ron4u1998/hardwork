    ...
    def table_data(self, image_path):
        ...
        for box, text in zip(boxes, texts):
            ...
            cv2.rectangle(image_boxes, (int(box[0][0]), int(
                box[0][1])), (int(box[2][0]), int(box[2][1])), (0, 0, 288), 1)
            cv2.putText(image_boxes, text, (int(box[0][0]), int(
                box[0][1])), cv2.FONT_ITALIC, 1, (222, 0, 0), 1)
        ...
        for box in boxes:
            ...
            cv2.rectangle(im, (x_h, y_h), (x_h+width_h,
                          y_h+height_h), (0, 0, 288), 1)
            cv2.rectangle(im, (x_v, y_v), (x_v+width_v,
                          y_v+height_v), (0, 288, 0), 1)
        ...
        for val in horiz_lines:
            cv2.rectangle(im_nms, (int(horiz_boxes[val][0]), int(horiz_boxes[val][1])), (int(
                horiz_boxes[val][2]), int(horiz_boxes[val][3])), (0, 0, 288), 1)
        ...
        for val in vert_lines:
            cv2.rectangle(im_nms, (int(vert_boxes[val][0]), int(vert_boxes[val][1])), (int(
                vert_boxes[val][2]), int(vert_boxes[val][3])), (288, 0, 0), 1)
...
# x = TableData()
# x.table_data('total_invoice/49.jpg')
